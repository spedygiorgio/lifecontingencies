...
\begin{enumerate}
  ...
  rate of 0.01 for the first cause of death. This rate can be changed via the
  \code{bottomCompletionSurvival} argument of \code{new("mdt", ...)} (default
  \code{0.99}, i.e.\ a 0.01 decrement rate, unchanged from previous releases);
  it only affects the synthetically reconstructed ages below the lowest age
...
\subsubsection{A worked example on real cause-of-death data, with a note on graduation (P4.2)}
...
cause-specific mortality report to a validated \code{mdt} object --- using
real data: Table 8 of \cite{ariasNVSR2013}, ``United States Life Tables
Eliminating Certain Causes of Death, 1999--2001'', published by the U.S.
National Center for Health Statistics. Table 8 tabulates, for 22 five-year
(and one open-ended) age bands from birth to 100+, the number of decrements
attributable to each of 33 causes of death out of a radix of $l_0 =
10{,}000{,}000$.

\paragraph{From an official report to a mutually exclusive partition.}
A \code{mdt} object requires its non-\code{x}/\code{lx} columns to be a
partition of total mortality: at every age the columns must sum to exactly
the decrement implied by \code{lx}. The 33 causes tabulated by
\cite{ariasNVSR2013}, however, are \emph{not} mutually exclusive: several
rows are parent categories that contain other rows as subtypes (e.g.\
``Malignant neoplasms'' contains site-specific cancers such as colon,
lung, breast and prostate), and others are cross-cutting aggregates that
overlap several disease groups (e.g.\ ``Alcohol-induced causes''). Summing
all 33 columns as if they were disjoint would double count decrements.
...
residual \code{other} category that closes the table to total mortality at
every age. The columns kept are \code{septicemia}, \code{hiv},
\code{cancer}, \code{diabetes}, \code{alzheimer}, \code{heart\_disease},
\code{hypertension}, \code{cerebrovascular}, \code{influenza\_pneumonia},
\code{copd}, \code{pneumonitis}, \code{liver\_disease}, \code{nephritis},
\code{congenital}, \code{accidents}, \code{suicide} and \code{homicide};
\code{other} is obtained, for each age band, as the total decrements minus
the sum of these 17 columns, and is non-negative at every age band, which
confirms no double counting remains.

We embed the resulting curated, mutually exclusive 5-year table below
(\code{partition\_sum} is the sum of the 17 named causes, kept only as a
diagnostic column to show that \code{partition\_sum + other == total\_dx}
at every age; it is dropped before building the \code{mdt}):

<<cdc.data3, keep.source=TRUE>>=
...
\paragraph{Graduating from 5-year bands to single years of age.}
The \code{mdt} class requires one row per single year of age, while
Table 8 is tabulated in 5-year bands. A modern, purpose-built way to
``ungroup'' banded counts into single-year counts is Eilers'
\emph{penalized composite link model} (PCLM, \cite{eilers2007pclm}),
implemented in the \pkg{ungroup} and \pkg{DemoTools} packages; it lets the
within-band shape --- including how the cause-of-death mix evolves within
a band --- be estimated smoothly and is generally the preferred approach
for this task.

\textbf{This vignette does not use PCLM.} \pkg{lifecontingencies} does not
depend on \pkg{ungroup} or \pkg{DemoTools}, and adding either as a
\code{Suggests} purely to compile one example was judged not worth the
extra dependency surface. Instead, we graduate \code{lx} with a monotone
cubic Hermite interpolant (the Fritsch--Carlson method,
\cite{fritschCarlson1980}), available in base \proglang{R} as
\code{stats::splinefun(..., method = "monoH.FC")}, and then split each
single-year total decrement across causes using \emph{each band's own,
...
reproducible, dependency-free approximation, not as a substitute for a
proper PCLM ungrouping. Because the split is proportional to a graduated
total that itself sums exactly across each band, the construction below
reaggregates \emph{exactly} back to the original 5-year, cause-specific
totals (checked below), which a naive graduation might not guarantee.

<<cdc.graduate, keep.source=TRUE>>=
...
\paragraph{Building and validating the \code{mdt} object.}
The single-year table \code{cdcSingle} (columns \code{x}, \code{lx} and
the 18 mutually exclusive causes) is now a valid input for \code{new("mdt",
...)}; because it already spans exactly $x = 0, \ldots, 100$ and closes
exactly to \code{lx[0]}, no meaningful synthetic bottom- or
top-completion is needed from \code{.tableSanitizer} (described earlier
in this section); at this scale, floating-point arithmetic can still make
the very last row's decrements sum to a value that differs from
\code{lx} by a fraction of a unit, in which case \code{.tableSanitizer}
adds one further, economically negligible completion row at $x = 101$
--- a harmless artifact of exact floating-point comparison, not a
data or modelling error.

<<cdc.build, keep.source=TRUE>>=
cdcMdt <- new("mdt",
...
We can now use the same \code{dxt}/\code{qxt}/\code{pxt} functions used
throughout this section on a real, cause-specific national life table.
For instance, at age 70:

<<cdc.compute, keep.source=TRUE>>=
...
decrements, illustrating how a \code{mdt} object built this way lets
standard \pkg{lifecontingencies} functions (\code{dxt}, \code{qxt},
\code{pxt}, \code{Axn.mdt}, \ldots) be applied directly to official,
cause-specific national mortality data, once the table has been reduced to
a mutually exclusive partition and graduated to single years of age.
