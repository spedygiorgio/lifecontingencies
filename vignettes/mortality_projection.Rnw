...
In the second part we will work with the StMoMo \cite{pkg:StMoMo} package for the same purposes, tha represents a more current approach to making mortality projections. 
...
The \pkg{StMoMo} provides many useful functions to manage mortality data.

\subsection{Loading data}

We will recycle previous example's data to initialize mortality data.
...
wxt <- genWeightMat(ages = ages.fit, 
  years = ita.StMoMoData.Ini$years,clip = 3)
@

\subsection{Fitting models}

The code below fits the Lee Carter models as well as the Reinshaw Haberban (RH) and the Cairn Blake Down (CBD) one \cite{Cairns2007} ones. Some are long to be fit (expecialli the RH one)
...
#fitting Reinshaw Haberman (long)
...
#save(list=c("LCfit","CBDfit","RHfit"),file="StMoMoModels.RData")
...
<<stmomo_lcplot,keep.source=FALSE, eval=TRUE>>=
plot(LCfit)
@

<<stmomo_rhplot,keep.source=TRUE>>=
plot(RHfit)
@

\subsection{Getting life tables}
